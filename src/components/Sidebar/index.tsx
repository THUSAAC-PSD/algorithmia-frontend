...
const Sidebar: React.FC<SidebarProps> = ({ activeItem, setActiveItem }) => {
    ...
    return (
        <div className={`${collapsed ? 'w-20' : 'w-72'} bg-slate-800 h-full flex flex-col transition-all duration-300 ease-in-out`}>
                ...
                <ul className="space-y-2">
                    {sidebarItems.map((item) => (
                        <li key={item.id}>
                            ...
                            >
                                <span className={`${activeItem === item.id ? 'text-indigo-400' : 'text-slate-400 group-hover:text-white'} transition-colors`}>
                                    {item.icon}
                                </span>
